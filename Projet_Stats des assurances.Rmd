...
```{r echo=FALSE, warning=FALSE, figures-side-biplot2, fig.show="hold", out.width="50%"}
par(mar = c(4, 4, .1, .1))
hist(RUC)
hist(log10(RUC))
```
...
```{r}
par ( mfcol =c(1 ,3))
boxplot ( Sinistre0 ~ cs , data =data2 )
boxplot ( Sinistre0 ~ agecat , data =data2 )
boxplot ( Sinistre0 ~ Acompm , data =data2 )
```
...
```{r}
# ne marche pas bien: Erreur dans pcor: x must be numeric
# Calcul de la matrice de corrélation partielle avec pcor
#cor_partielle <- pcor(data2, method = "pearson")

# Affichage de la matrice de corrélation partielle
#print(cor_partielle)
...
```{r echo=FALSE, figures-side-hist-NSin, fig.show="hold", out.width="50%"}
hist(NSin)
hist(log(NSin))
```
